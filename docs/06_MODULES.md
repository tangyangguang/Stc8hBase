...
SPI 第一版采用硬件 SPI 主机轮询实现，只定义基础收发能力，不引入软件 SPI 双实现。默认使用 P1.3/P1.4/P1.5 引脚组，硬件 SS 使用 `SSIG=1` 忽略，片选由板级或应用代码自行控制。HAL 默认把 MOSI/MISO/SCLK 保持为准双向，并显式启用当前引脚组的 MISO 数字输入。
