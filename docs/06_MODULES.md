...
SPI 第一版采用硬件 SPI 主机轮询实现，只定义基础收发能力，不引入软件 SPI 双实现。默认使用 P1.3/P1.4/P1.5 引脚组，硬件 SS 使用 `SSIG=1` 忽略，片选由板级或应用代码自行控制。HAL 默认把 MOSI/MISO/SCLK 保持为准双向，释放 MISO 锁存，并显式启用当前引脚组的 MISO 数字输入。

## 4. Drivers

Drivers 支持外部器件和常见板级外设。

nRF24L01 驱动只封装芯片 SPI 协议和 CE/CSN 时序；CE/CSN 端口模式由板级 `DRV_NRF24L01_CONFIGURE_PINS()` hook 配置。
...
| `drv_nrf24l01` | nRF24L01/nRF24L01+ SPI 射频模块芯片驱动 |
...
`drv_nrf24l01` 只封装芯片寄存器、FIFO、IRQ、TX/RX 和 nRF24L01+ 可选 ACK payload/dynamic payload 能力，不包含配对、频道扫描、遥控协议或业务 payload 格式。

## 5. Protocols

Protocols 模块封装多个应用项目复用的通信协议，不直接理解业务字段。

| 模块 | 作用 |
| --- | --- |
| `proto_rf_link` | 固定 32 字节小包、序号、确认、心跳和状态回传的通用 RF 链路层 |

`proto_rf_link` 第一版只提供固定频道可靠小包格式，不做绑定持久化、频道扫描、路由、mesh 或加密。命令、配置、遥控和传感器数据均作为业务 payload 由应用解释。
