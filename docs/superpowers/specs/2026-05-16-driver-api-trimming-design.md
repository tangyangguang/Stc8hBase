...
`display_digits` requires `encode_digit`; `display_number` requires `encode_digit` and owns the divide-by-10 helper. Invalid macro combinations fail at compile time.
