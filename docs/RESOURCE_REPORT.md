...
- `delay_us_probe`、`h8k64u_soft_reset_probe`、`h8k64u_iap_exec_probe` 一次性 probe 示例已删除；长期硬件验证只保留文档清单和正式示例入口。
- `h8k64u_ota_min_app` 删除内部 `entry_probe` 环境，默认只构建非写擦应用；mark-valid IAP 路径由 full 检查显式构建。
