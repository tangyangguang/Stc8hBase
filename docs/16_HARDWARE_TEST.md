...
examples/platformio/nrf24_uart_diag/.pio/build/STC8H1K08/firmware.hex
...
`nrf24_uart_diag` 使用上表 nRF24 引脚和 UART1，不依赖 LED、TM1637、按键、电机或接收端固件。
...
- 当前硬件验收固件上电后先依次显示 `8888`、`0123`、`4567`，随后从 `0000` 起递增。
- 串口周期输出 `tm1637 ok`。
- 未接模块或 DIO/CLK 接错时，通常输出 `tm1637 error`。

说明：

- TM1637 不是标准 I2C。这里记录的是当前临时实测接线，不是推荐默认值。
- 当前接线 DIO/SDA=P3.2、CLK=P1.7，与 LCD1602 软件 I2C 引脚相同；测试 TM1637 时不要同时驱动 LCD1602。
- CLK/DIO 使用开漏输出，并启用 P1/P3 内部上拉；如模块已有上拉也可工作。
- 当前接线不占用 SPI 默认引脚 P1.3/P1.4/P1.5。
...
- 协议自检版本已烧录成功。
- 串口 115200 已读到连续 `ir nec ok`。
- 降频版本已复烧成功。
- 串口 115200 在约 4 秒内读到 2 行 `ir nec ok`，输出不再刷屏。
...
- 会构建 EEPROM 写擦测试，但不自动上传；当前芯片 ISP 识别为 `EEPROM flash: 0.5 KB`，破坏性示例只用于已确认可擦的 512 字节 EEPROM 页。
...
5. Build and flash `examples/platformio/h8k64u_adc_read` with `ADC_VRef+` connected; monitor UART1 at 115200 baud and expect repeated `ADC0=0x.... ADC15=0x.... VREF+=....mV`.
...
7. After confirming the 512-byte EEPROM page can be erased, build and flash `examples/platformio/h8k64u_eeprom_rw`; monitor UART1 at 115200 baud and expect `H8K64U EEPROM ok`.
8. Build and flash `examples/platformio/h8k64u_wdt_feed`; monitor UART1 at 115200 baud and expect `H8K64U WDT feed`.
...
- `h8k64u_adc_read` 已扩展为同时读取 ADC0 和 ADC15；UART1 115200 监视到 `ADC15=0x05BC/0x05BD/0x05BE`，按芯片出厂内部参考 `1191mV` 反推 `VREF+=3318..3322mV`。`ADC_VRef+` 已接 3.3V，因此内部参考反推供电测试通过。ADC0 未接已知外部电压，外部通道绝对精度仍需另接电压源复测。
...
- 2026-06-19 后续授权破坏性实验：已将测试芯片 `program_eeprom_split` 改为 `512`。在该配置下，UART1 OTA smoke 可完成 `BEGIN/WRITE/VERIFY/COMMIT`，但 OTA 写入后的 app 未打印 `H8K64U OTA app v1.0.0`；对照实验中，同一 app 增加 `0x0000 -> 0x0200` reset vector 后通过 ISP 直接烧录，可以正常打印该 banner。结论：app 与 `0x0200` 执行本身可用，当前失败集中在“用户 bootloader 通过 IAP 写入的数据是否进入 CPU 可执行视图”。
