...
当前 `stc8h_pwm` 第一版只实现 `PWMA` 1..4 基础输出，默认 P1 引脚组。`PWMA` 1..4 共用同一个 ARR 周期，不能在同一 `PWMA` 组内同时使用不同 PWM 频率；`stc8h_pwm_init_period()` 和 `stc8h_pwm_init()` 都会更新这个共享周期。

`stc8h_pwm_init_period()` 在已有 `PWMA` 输出使能且新周期不同于当前周期时返回 `STC8H_ERROR`，防止运行中静默改掉其他通道频率。需要切换共享周期时，应用必须先禁用同组输出，再重新初始化周期和通道。
