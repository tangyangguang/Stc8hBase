...
USB、DMA、RTC、LCM、UART4、完整 I/O 中断支持、RS485 协议和 433MHz 模块驱动不在当前已实现支持范围内。OTA 设计中计划复用 UART2/UART3 做 RS485 和 433 串口透传适配；具体 RS485 半双工控制和 433 串口透传可靠性仍需单独实现和验证。SPI/寄存器型 433 射频芯片不在当前支持范围，也不列入 OTA 基础能力计划。
