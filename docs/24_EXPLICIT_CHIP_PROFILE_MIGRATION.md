...
Existing projects that already define `STC8H_CHIP_STC8H1K08 1` can continue to build because unspecified chip profile macros default to `0`. Adding explicit negative chip macros such as `STC8H_CHIP_STC8H8K64U 0` is still recommended in board configuration files for readability.
