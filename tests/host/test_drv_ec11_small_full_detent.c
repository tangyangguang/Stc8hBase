...
#define DRV_EC11_SMALL_STEPS_PER_DETENT 4
...
{
    return drv_ec11_scan_delta_small(ec11, (stc8h_u8)((state >> 1) & 1u), (stc8h_u8)(state & 1u));
}
...
int main(void)
{
    ...
    failures = 0;
    failures += expect_delta("forward full detent", forward_full, sizeof(forward_full), 1);
    failures += expect_delta("reverse full detent", reverse_full, sizeof(reverse_full), -1);
    failures += expect_delta("forward bounce before detent", forward_with_bounce, sizeof(forward_with_bounce), 1);
    failures += expect_delta("forward missing phase", forward_missing_phase, sizeof(forward_missing_phase), 1);
